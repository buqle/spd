...
import { Form, Row, Col, DatePicker, Input, Select, Button, Icon, message,Badge  } from 'antd';
  ...
  render() {
    ...
    const columns = [
    {
      ...
      }, 
      {
        ...
        width: 200,
        render: (text, record) => {
            return  <Badge count={record.commonCheckBillDetailListCount} overflowCount={999} style={{right:'-26px',zIndex:'0',top: '8px'}} showZero><Link to={{ pathname: `/purchase/checkDecrease/inventoryAudit/details/${record.checkBillNo}`}}>{text}</Link></Badge>
        }
      },
      {
        title: '状态',
