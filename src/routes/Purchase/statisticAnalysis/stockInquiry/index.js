...
import { Form, Row, Col, Button, Tooltip, Select, Icon, message ,Input,DatePicker} from 'antd';
...
class StockInquiry extends PureComponent {
  ...
  handleSearch = (e) => {
    ...
    this.props.form.validateFields((err, values) => {
      ...
      if(!err){
          if(this.state.showButtton2){
              const productTime = values.productTime === undefined ? moment().format('YYYY-MM-DD') : values.productTime;

              if (productTime) {
                  values.recordDate = productTime.format('YYYY-MM-DD');
                  this.setState({
  ...
  toggle = () => {
    ...
    VerfiyTableCallBack = (data) =>{
        this.setState({
            stromNum:data[0].storeNum
        })
    }
    callcb(value){
      console.log(value)
  ...
  render() {
    ...
    let tt=!this.state.recordDate?this.state.query.recordDate:this.state.recordDate
    const columns = [
     {
        title: '序号',
        width: 60,
      ...
      },   
     {
        ...
        render:(text, record)=>(
          <span>
              {showButtton?<Link to={{pathname: `${match.path}/details/${record.deptCode}/${record.drugCode}/${record.hisDrugCode}`}}>
                  <Tooltip placement="topLeft" title={text}>{text}</Tooltip>
              </Link>:null}
              {showButtton2?<Link to={{pathname: `${match.path}/details/${record.deptCode}/${record.drugCode}/${record.hisDrugCode}/${tt}`}}>
                  <Tooltip placement="topLeft" title={text}>{text}</Tooltip>
              </Link>:null}
          </span>
        )
      ...
      },
      {
        ...
        width: 148,
            ...
            width: 138,
            render:(text)=>(
                <div className='requests' style={{width:104}}>
                    <Tooltip placement="topLeft" title={text}>{text}</Tooltip>
                </div>
            )
        }
    ...
    let query = this.props.base.queryConditons;
        query = {...query,recordDate:!this.state.recordDate?this.state.query.recordDate:this.state.recordDate};
    const {deptList, purchaseTypeList,stromNum,showButtton2,showButtton} = this.state;
    return (
          ...
          <Row gutter={30}>
            ...
            <Col span={8}>
                    ...
                    <Col span={8}>
                        <FormItem label={`入库日期`} {...formItemLayout}>
                            {getFieldDecorator('productTime',{
                                ...
                                initialValue:moment(this.state.query.recordDate),
                                rules:[
...

              {
                  stromNum?
                      <Col span={8}>
                          <FormItem label={`库存总数`} {...formItemLayout}>
                              <Input  disabled  value={stromNum}/>
                          </FormItem>
                      </Col>:null
              }
        ...
        <Button  type='default' onClick={this.export}>导出</Button>
          {
              showButtton2?
                  <RemoteTable
                      ...
                      url={statisticAnalysis.QUERY_DRUG_DEPT_LIST_NEW}
                      isJson={true}
                      query={query}
                      ref="tab"
                      style={{marginTop: 20}}
