...
export default {
  ...
  effects:{
    ...
    *commonConfirmCheck({ payload, callback },{ put, call }) {
      ...
      //药库 - 拒收验收
      *commonReject({ payload, callback },{ put, call }) {
          const data = yield call(base.commonReject, payload);
          if(callback && typeof callback === 'function') {
              callback(data)
          };
      },
    ...
    *applySubmit({payload, callback}, {call}) {
      ...
      //药品申领 - 保存草稿
      *applySaveDraft({payload, callback}, {call}) {
          const data = yield call(pharmacy.applySaveDraft, payload);
          if(data.code !== 200) {
              message.error(data.msg);
              return;
          };
      ...
      //药品申领 - 草稿详情
      *applyEdit({payload, callback}, {call}) {
          const data = yield call(pharmacy.applyEdit, payload);
          if(data.code !== 200) {
              message.error(data.msg);
              return;
          };
    ...
    *submitBackStorage({ payload,callback },{ call }){
      ...
      }else if(data.code == 200&&data.length>1){
          return message.error(data.data[1])
      }
      if(callback) callback(data.data)
    },

      // 退货 编辑草稿详情
      *editDraft({ payload,callback },{ call }){
          const data = yield call(outStorageService.editDraft, payload);
          if(data.code !== 200){
              return message.error(data.msg||'获取草稿单详情失败')
          }
          if(callback) callback(data.data)
      },
      // 退货 编辑草稿头部详情
      *editDraftHead({ payload,callback },{ call }){
          const data = yield call(outStorageService.editDraftHead, payload);
          if(data.code !== 200){
              return message.error(data.msg||'获取草稿单详情失败')
          }
          if(callback) callback(data.data)
      ...
      // 退货 保存草稿
      *saveDraft({ payload,callback },{ call }){
          const data = yield call(outStorageService.saveDraft, payload);
          if(data.code !== 200){
              return message.error(data.msg||'保存草稿失败')
          }
          if(callback) callback(data.data)
      },
      ...
      *submitDraft({ payload,callback },{ call }){
          const data = yield call(outStorageService.submitDraft, payload);
          if(data.code !== 200){
              return message.error(data.msg||'提交草稿失败')
          }
          if(callback) callback(data.data)
      },
    ...
    *depotplanDetailExport({ payload,callback },{ call }){
      ...
      //补货计划导出
      *depotplanExport({ payload,callback },{ call }){
          const data = yield call(base.depotplanExport, payload);
          if(callback && typeof callback === 'function') {
              callback(data);
          };
      },
     ...
     *depotBackSubmit({ payload,callback },{ call }){
      ...
      *getAllDeptByCondition({payload, callback}, {call}) {
          const data = yield call(base.getAllDeptByCondition, payload);
          if(data.code === 200 && data.msg === 'success') {
              callback && callback(data.data);
          }else {
              message.error(data.msg);
      ...
      *getLogCountByDate({ payload, callback },{ put, call }){
          const data = yield call(base.getLogCountByDate, payload);
          if(callback && typeof callback === 'function'){
              callback(data);
          }
      },
      //修改指示货位进行移库操作
      *locAdjustStock({ payload, callback },{ put, call }){
          const data = yield call(base.locAdjustStock, payload);
          if(data.code !== 200){
              message.error(data.msg)
          }
      ...
      //药房 - 确认召回
      *queryDrug({ payload,callback },{ call }){
          const data = yield call(base.queryDrug, payload);
          if(data.code !== 200){
              return message.error(data.msg||'操作失败')
          }
          if(callback) callback(data.data)
      ...
      //删除申领单
      *deleteAppStore({payload, callback}, {call}) {
          const data = yield call(base.deleteAppStore, payload);
          console.log(data, '删除');
          if(data.code === 200 && data.msg === 'success') {
              callback && callback(data);
          }else {
