...
class AddSupplementDocuments extends PureComponent{
  ...
  //提交
  onSubmit = () =>{
    const {dataSource, typeValue, makeupCause,deptValue,deptCode} = this.state;
    ...
    if(!typeValue) {
      ...
      if(typeValue&&typeValue==5) {
          if(!deptValue){
              return message.warning('请选择科室');
          }
      };
    if(!makeupCause.trim()) {
    ...
    Conform({
      ...
      onOk:()=>{
        ...
        postData.ctdCode=deptValue;
          postData.ctdName=deptCode;
  ...
  setRowInput = (val,field,index,isDate)=>{
    ...
    if(val>ds[index].usableQuantity){
        val=1;
        ds[index][field]=1;
        this.setState({dataSource:ds})
        return message.warning('数量不得可用库存!');
    }
    ds[index][field]=val
    console.log(ds)
    this.setState({dataSource:ds})
  ...
  typeChange = (value) => {
    this.setState({
      ...
      if(value==5){
          this.props.dispatch({
              type: 'base/getAllDeptByCondition',
              callback: (data) => {
                  const dataList = data.map(item => ({
                      ctdCode: `${item.ctdCode}`,
                      ctdName: item.ctdName,
                  }));
                 this.setState({
                     deptList: dataList,
                 })
  ...
  //科室下拉
    ...
    if(option){
        this.setState({
            deptValue: value,
            deptCode: option.props.children
        });
                    ...
                    <div className="ant-form-item-control-wrapper ant-col-xs-24 ant-col-sm-19">
                        <div className='ant-form-item-control'>
                            ...
                            >
                                {
                                    deptList.map(item => (
                                        <Option key={item.ctdName} value={item.ctdCode}>{item.ctdName}</Option>
                                    ))
                                }
                            </Select>
                        </div>
