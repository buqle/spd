...
import { Form, Row, Col, Button, Icon, Select , Input ,DatePicker, Tooltip,Badge } from 'antd';
...
const columns = [
{
      ...
      },   
  {
    ...
    render: (text, record) => 
    <span>
        <Badge count={record.backStoreDetailItemCount} overflowCount={999} style={{right:'-22px',zIndex:'0',top:4}} showZero>
            {
                record.isDraft==1?<Link to={{pathname: `/drugStorage/outStorage/backStorage/details/${record.backNo}/${record.isDraft}`}}>{text}</Link>:
                  <Link to={{pathname: `/drugStorage/outStorage/backStorage/details/${record.backNo}/2`}}>{text}</Link>
