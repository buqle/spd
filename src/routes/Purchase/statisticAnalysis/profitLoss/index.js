...
class ProfitLoss extends PureComponent {
  ...
  _tableCallback = (data) => {
      let total = data.map((item) => item.excessiveTotalMoney);
      let totals=0;
      total.forEach(ele => {
          totals += ele;
      });
      this.setState({
          excessiveTotalMoney:totals
      })
  }
  export = () => {
    ...
    const columns = [
     {
      ...
      }, 
      {
        ...
        title: '实际报益金额',
        dataIndex: 'overflowAmount',
        width: 138,
      }
    ];
    const {excessiveTotalMoney} = this.state;
      ...
      <div className='ysynet-main-content'>
        ...
        <RemoteTable
          ...
          footer={() => (
            <Row>
              <Col>
                <div className="ant-form-item-label-left ant-col-xs-10 ant-col-sm-4">
                  <label style={{fontWeight: 600}}> {dept?'部门实际总损益金额(元)':'实际总损益金额(元)'}</label>
                </div>
                <div className="ant-form-item-control-wrapper ant-col-xs-10 ant-col-sm-7">
                  <div style={{color: 'red'}} className='ant-form-item-control'>{excessiveTotalMoney || 0}</div>
                </div>
              </Col>
            </Row>
          )}
