...
export default {
  ...
  effects: {
    ...
    *excessiveExport({callback, payload}, {call}) {
      ...
      //盘点新增批号
      *addNewLot({callback, payload}, {call}) {
          const data = yield call(checkDecrease.addNewLot, payload);
          if(callback && typeof callback === 'function') {
              callback(data);
          };
      },
      //盘点新增批号－获取货位
      *findLocInfo({callback, payload}, {call}) {
          const data = yield call(checkDecrease.findLocInfo, payload);
          if(callback && typeof callback === 'function') {
              callback(data);
          };
      },
