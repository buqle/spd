...
class NewAdd extends PureComponent {
  ...
  componentDidMount (){
      let {applyCode,status} =this.props.match.params;
      this.setState({
          disStatus:status
      })
      if(status==0){
      ...
      this.getReplenishList(status.toString());
      this.props.dispatch({
          ...
          callback: (data) => {
              this.setState({
                  applyTypeList: data,
                  applyType:status!=0? '1':this.state.applyType,        //补货方式
              });
          }
      });
    ...
    getDetail = () => {
        ...
        this.props.dispatch({
            type:'base/applyEdit',
            payload: { applyCode,applyStatus:status},
            callback:(data)=>{
                console.log(data)
                this.setState({
                    dataSource:data.detailList,
                    applyType:data.applyType.toString(),
                    query: {
                        ...query,
                        deptCode: data.distributeDeptCode,
    ...
    dispatch({
      ...
      callback: (data) =>{
        this.setState({ 
          ...
          query: {
            ...query,
            deptCode: type === '1' &&status!=0? data[0].id :status==0?this.state.query.deptCode:undefined
          },
        })
      }
    });
  ...
  handleOk = () => {
    ...
    this.props.dispatch({
      ...
      callback: (data) => {
          let filterArr= _.filter(data,(o)=>{
              return o.usableQuantity!=0
          })
          let filterArr2= _.filter(data,(o)=>{
              return o.usableQuantity==0
          })
        this.setState({
          dataSource: [...this.state.dataSource, ...filterArr],
          btnLoading: false,
          visible: false,
          modalSelected: [],
          modalSelectedRows: []
            ...
            if(filterArr2.length){
                message.warning('药库库存为0产品将不进行添加');
            }
        })
  ...
  submit = (applyStatus) => {   //提交  保存
    let {dataSource, applyType, query,selectedRows} = this.state;
    ...
    this.setState({
      saveLoading: applyStatus
    });
    let body = {
      applyStatus:applyStatus==1?applyStatus:0,
      applyType,
      distributeDeptCode: query.deptCode,
      detaiList: detailListVo,
      applyCode: query.applyCode
    };

    this.props.dispatch({
      type: applyStatus==1?'base/applySubmit':'base/applySaveDraft',
      payload: body,
      callback: ()=>{
        message.success(`${applyStatus == 2? '保存草稿' : '提交'}成功`);
          history.push({pathname:"/pharmacy/wareHouse/drugsFor"})
  ...
  render() {
      ...
      , {
            ...
            render: (text, record, index) => {
                return <InputNumber
                    ...
                    onChange={(value)=>{
                        ...
                        dataSource[index].applyNum = value;
        ...
        title: (
          <Tooltip title="当前部门近7天的药品发药量">
            <span>7天使用量</span>
              <b className='typecolor' style={{fontSize:11}}>(已向上取整)</b>
          </Tooltip>
        ),
    ...
    return (
      <div className="fullCol" style={{ padding: 24, background: '#f0f2f5' }}>
          ...
          <Row gutter={30}>
            <Col md={12} lg={8} xl={6}>
              <FormItem {...formItemLayout} label="补货方式">
                <Select
                  showSearch
                  disabled={dataSource.length === 0||disStatus==1&&dataSource.length === 0? false : true}
                  style={{width: "100%"}}
                  onChange={(value) => {
                    this.setState({
                      applyType: value
        ...
        >
          <Row>
            <Col style={{ marginLeft: 4,display: 'flex', alignItems: 'baseline' }}>
              <FetchSelect
                value={query.hisDrugCodeList[0]}
                style={{ width: 496,marginRight:14 }}
                allowClear={true}
                placeholder='药品名称'
              ...
              />
                {
                  showTit?<p style={{fontWeight:'bold',color:'#FF9800',fontSize:16}}>以下背景色标红为药库库存为0的产品</p>:null
                }
            </Col>
          </Row>
          <div className='detailCard detailCard-dis'>
            <RemoteTable
              query={query}
        ...
        >
          ...
          />
          {
            ...
            <Col style={{ textAlign: 'right', marginTop: '10px' }}>
              <Button loading={saveLoading==1?true:false} onClick={()=>{this.submit(1)}} type='primary'  style={{ marginRight: 8 }}>提交</Button>
                <Button loading={saveLoading==2?true:false} onClick={()=>{this.submit(2)}} type='primary'  style={{ marginRight: 8,marginLeft:8 }} >保存草稿</Button>
                <Button type='primary' ghost>
                    <Link to={{pathname:`/pharmacy/wareHouse/drugsFor`}}>取消</Link>
                </Button>
            </Col>
          }
        </div>
