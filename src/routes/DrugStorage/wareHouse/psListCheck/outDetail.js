...
class PslistCheck extends PureComponent{
    ...
    saveCheck = (flag) => {
        ...
        this.setState({checkLoading:flag==1?1:2});
        ...
        this.props.dispatch({
            type: flag!=1?'base/commonConfirmCheck':'base/commonReject',
            payload: {
                detailList,
                distributeCode: this.state.id,
                checkType: 1,
                type:flag==1?209:detailInfo.type
            },
            callback: ({data, code, msg}) => {
                ...
                message.success(flag!=1?'确认验收成功':'拒收成功');
                this.setState({
                    selected: []
                });
               if(flag!=1){
            ...
            <div className='fullCol fadeIn'>
                ...
                <Row style={{display: 'flex', alignItems: 'center'}}>
                  <Col span={12} style={{ marginLeft: 4 }}>
                    <FetchSelect
                      ...
                      placeholder='药品名称'
                      deptStateb={' '}
                      url={TableSearchUrl.searchwareHousepsListCheck}
                      cb={(value, option) => {
                ...
                <div className='detailCard'>
                    <Tabs
                        ...
                        tabBarExtraContent={
                          detailInfo.checkDetailStatus === 1 ||detailInfo.checkDetailStatus === 0 ?
                              <div>
                                  {
                                      detailInfo.type==106?
                                          <Button loading={checkLoading==1?true:false} type='primary' onClick={this.saveCheck.bind(this, 1)} style={{ marginRight: 8 }}>拒收</Button>:null
                                  }
                                  <Button loading={checkLoading==2?true:false}  type='primary' onClick={this.saveCheck}>确认验收</Button>
                              </div>
