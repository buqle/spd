  ...
  render(){
    ...
    const columns = [
      ...
      },   
      {
        ...
        width: 178,
        render:(text,record)=>(
            <Badge count={record.pickingDetailItemCount} overflowCount={999} style={{right:'-18px',zIndex:'0',top: '8px'}} showZero>
            <Link to={{pathname: `/pharmacy/outStorage/pickingUnderShelve/details/${text}/${record.pickingType}`}}>{text}</Link>
            </Badge>
        )
      },
      {
