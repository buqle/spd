...
import { Form, Row, Col, DatePicker, Input, Select, Button, Icon,Badge } from 'antd';
  ...
  render() {
    ...
    const columns = [
    {
      ...
      },    
      {
        ...
        width: 196,
        render: (text, record) => {
          return <Badge count={record.commonCheckBillDetailListCount} overflowCount={999} style={{right:'-22px',zIndex:'0',top: '8px'}} showZero><Link to={{ pathname: `/pharmacy/checkDecrease/profiLossRecord/details/checkBillNo=${record.checkBillNo}&causticExcessiveNo=${record.causticExcessiveNo}`}}>{text}</Link></Badge>
        }
      },
      {
        title: '盘点单',
