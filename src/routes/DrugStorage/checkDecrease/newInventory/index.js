    ...
    const columns = [
     {
      ...
      },  
      {
        ...
        render: (text, record) =>(
            <span>
              <Badge count={record.commonCheckBillDetailListCount} overflowCount={999} style={{right:'-27px',zIndex:'0',top: '8px'}} showZero>
                  <Link to={{ pathname: `/drugStorage/checkDecrease/newInventory/details/${record.checkBillNo}`}}>{text}</Link>
              </Badge>
            </span>
        )
      },
