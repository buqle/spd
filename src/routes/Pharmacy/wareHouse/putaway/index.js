...
import {Form, Input , Icon , DatePicker , Row, Col, Button, Select,Badge } from 'antd';
    ...
    const columns = [
     {
      ...
      },  
      {
        ...
        width: 178,
        render:(text, record)=>(
            <Badge count={record.checkAcceptDetailsItemsCount} overflowCount={999} style={{right:'-18px',zIndex:'0',top: '8px'}} showZero>
            <Link to={{pathname: `/pharmacy/wareHouse/putaway/details/code=${text}&status=${record.auditStatus}`}}>{text}</Link>
            </Badge>
        )
       },
      {
