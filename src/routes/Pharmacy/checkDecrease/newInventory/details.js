...
import {Row, Col, Input, Button, message, Tooltip, DatePicker, InputNumber, Radio, Select} from 'antd';
...
class Details extends PureComponent {
    ...
    onCheck = () => {
        ...
        let isNull = includeChildren.every((item,index,array) => {   //是否为空
            if(!item.practicalRepertory&&item.practicalRepertory!=0) {
                message.error('实际库存不能为空');
                return false;
            }else if(!item.practicalBatch) {
                message.error('实际批号不能为空');
                return false;
            }else if(!item.locCode){
                message.error('货位不能为空');
                return false;
            }else if(!item.realProductTime) {
                message.error('实际生产日期不能为空');
                return false;
            }else if(!item.validEndTime) {
    ...
    addBatch = (record, i) => {
        ...
        this.props.dispatch({
            type: 'checkDecrease/addNewLot',
            payload: {
                bigDrugCode: record.bigDrugCode
            },
            callback: (data) => {
                data.data.dataVo.forEach(function (value,index,arr) {
                    dataSource[i].accc=[]
                    dataSource[i].accc.push(data.data.dataVo)
                })


                this.props.dispatch({
                    type: 'checkDecrease/findLocInfo',
                    payload: {
                        unitSort:record.accc[0][0].unitSort,
                        unitCode:record.accc[0][0].unitCode
                    },
                    callback: ({data}) => {
                        ...
                        dataList.forEach(function (value,index,arr) {
                            dataSource[i].positionName=[]
                            dataSource[i].positionName.push(dataList)
                        })

                        /*const dataList2 = data.map(item => ({
                            id: `${item.id}`,
                            locationName: item.locationName,
                        }));
                        dataList2.forEach(function (value,index,arr) {
                            dataSource[i].locationName=[]
                            dataSource[i].locationName.push(dataList2)
                        })*/

                        if( dataSource[i].accc){
                            dataSource[i].children = dataSource[i].children || [];
                            //dataSource[i].children[i].positionName=dataSource[i].positionName
                            ...
                            dataSource[i].children.push({
                                ...
                                showUint:'show',
                                practicalBatch:null,
                                unit:record.accc[0][0].unit,
                                locCode:record.positionName[0][0].id,
                                unitCode:record.accc[0][0].unitCode,
                                //货位默认locName:record.positionName[0][0].positionName,
                                positionTypeName:record.positionName[0][0].locationName
                            });
                            this.setState({
                                tableLoading:false
                            })
                            //dataSource[i].children[i].positionName=dataSource[i].positionName
                            console.log(dataSource[i].positionName)
                            console.log(dataSource[i].children)
    ...
    //更改货位
    handleLonameChange=(value,i, key, record)=>{
        let {dataSource} = this.state;
        if(!record.id) {
            let index;
            dataSource.map((item, pIndex)=>{
    ...
    handleProvinceChange = (value,option) =>{
        let {dataSource} = this.state;
        if(option){
            let {i,key,record,un,children}=option.props
            if(!record.id) {
                record.positionName[0][0].locationName=''
                let index;
                dataSource.map((item, pIndex)=>{
                    if(item.id === record.pId) {
                        index = pIndex
                ...
                /*dataSource[i].locationName=[]
                dataSource[index].children[i].locationName=[]*/
                ...
                this.props.dispatch({
                    ...
                    callback: ({data}) => {
                        ...
                        dataSource[index].children[i].positionName=[]
                        dataSource[index].children[i].positionName=dataSource[i].positionName
                        dataSource[index].children[i].positionTypeName =dataSource[i].positionName[0][0].locationName;
                        dataSource[index].children[i].locCode =dataSource[i].positionName[0][0].id;
                        //货位默认 dataSource[index].children[i].locName =dataSource[i].positionName[0][0].positionName;
                        /*   const dataList2 = data.map(item => ({
                               id: `${item.id}`,
                               locationName: item.locationName,
                           }));
                           ...
                           dataSource[index].children[i].locationName=dataSource[i].locationName*/

                        /*dataSource[index].children[i].positionName = dataList;
                        dataSource[i].positionName = dataList;*/
                        console.log(dataSource[i])
                    }
                })
                dataSource[index].children[i].unitCode = value;
            }else {
                dataSource[i].unitCode = value;
                //dataSource[i].positionName = this.state.positionName;
            };
            this.setState({
                dataSource: [...dataSource]
            });
    ...
    render() {
        ...
        let columns = [
            {
                ...
                render:(text,record,i)=>{
                    if(record.showUint&&info.checkStatus === 2 && record.checkDetailStatus === 1){
                        if(record.positionName[0][0].locationName){
                            return <Select
                                showSearch
                                placeholder={'请选择'}
                                onChange={(value) => {
                                    this.handleLonameChange(value, i, 'locCode', record);
                                }}
                                defaultValue={record.positionName.length?record.positionName[0][0].positionName:'加载中...'}
                            >
                                {
                                    record.positionName.length&&record.positionName.map( items => (
                                        items.map( item => (
                                            <Option key ={item.id} value={item.id}>{item.positionName}</Option>
            ...
            },
            {
                ...
                render:(text,record,i)=>{
                    if(record.showUint&&info.checkStatus === 2 && record.checkDetailStatus === 1){
                        return  record.positionName.length&&record.positionName[0][0].locationName?record.positionName[0][0].locationName:'加载中...'
                    }else {
                        return text
                    }
                }
                ...
                render:(text,record,i)=>{
                    if(record.showUint&&info.checkStatus === 2 && record.checkDetailStatus === 1){
                        return <Select
                            ...
                            defaultValue={record.accc[0][0].unit}
                            onChange={this.handleProvinceChange}
                            filterOption={(input, option) => option.props.children.indexOf(input) >= 0}

                        >
                            {
                                record.accc.map( items => (
                                    items.map( item => (
                                        <Option key ={item.unitSort} value={item.unitCode} i={i}  record={record} un={item.unitSort}>{item.unit}</Option>
        ...
        return (
            <div className='fullCol fadeIn'>
                ...
                <div className='detailCard detailCard-cocy'>
                    <Row>
                        <Col span={12}>
