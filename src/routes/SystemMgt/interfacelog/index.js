...
class RecallAndLocked extends PureComponent {
    ...
    //重新发送接口
    sendMenthod=({id},event)=>{
        let {query } = this.state;
        if(event.target.value==id){
            this.setState({loadingB:id});
        }
        Conform({
            content:"您确定要执行此操作？",
            onOk:()=>{
            ...
            onCancel:()=>{this.setState({ loadingB: false });}
        })
    ...
    render() {
        const { loading,loadingB} = this.state;
                ...
                render:(text,record)=>(
                    <div>
                        ...
                        {record.isSupportSend==0?
                            <Button loading={loadingB&&record.id==loadingB?true:false} type="primary" onClick={this.sendMenthod.bind(this,{id:record.id})} style={{ margin:'0  8px'}}  value={record.id}>重发</Button>
                            :<Button type="dashed" disabled style={{ margin:'0  8px'}} >重发</Button>
                        }
                        {record.isHandle==1&&record.resultCode!=0?<Button loading={loading&&record.id==loading?true:false} type="primary" onClick={this.handleLog.bind(this,{id:record.id})} value={record.id}>处理完毕</Button>:<Button type="dashed" disabled>处理完毕</Button>}
                    </div>
                )
            }
        ];
