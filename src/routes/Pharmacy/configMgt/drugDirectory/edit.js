...
import { Form , Row , Button , Col , Select , Input , Modal , Collapse , message, InputNumber, Table, Radio, Icon, Tooltip ,Checkbox} from 'antd';
        ...
        this.props.dispatch({
            ...
            callback:(data)=>{
                ...
                const goodsListMap = {
                    ...
                    locStore: {
                        goods: 'alternativeLoc',
                        tip: '单位默认补货单位',
                        unit: fillBackData.alternativeLocName,
                        name: '备选上架货位',
                    }
                };
                ...
                this.setState({
                    ...
                    adjust:{
                        ...
                        sourceLocCode:data.data.replanStore
                    }
                })
                this.removeLocUnit(data.data.replanStore)
                goodsList.push({
                    name: goodsListMap.locStore.name,
                    dataIndex: goodsListMap.locStore.goods,
                    //dataIndexUnit: goodsListMap.locStore.unit,
                    //list: goodsVos[0].dispensing,
                    tip: goodsListMap.locStore.tip
                });
                //获取补货单位下拉框
                this.props.dispatch({
            ...
            onOk:()=>{
                this.props.form.validateFields((err,values)=>{
                    if(!err){
                        ...
                        if (!isNull) return;
                        if(values.replanStore==values.alternativeLoc){
                            message.error('指示货位与备选货位不能一致!')
                            return false;
                        }
                        const {
                            replanStore,
                        ...
                        let postData = {
                            ...
                            drugInfo:{
                                ...
                                upperQuantity:upperQuantity?upperQuantity:0,
                                downQuantity:downQuantity?downQuantity:0,
                                replanStore,
...
//获取备选上架货位
    removeLocUnit = (locId) => {
        ...
        this.props.dispatch({
            type:'drugStorageConfigMgt/AlternativeLoc',
            payload:{
                deptCode:fillBackData.deptCode,
                unitCode:fillBackData.replanUnitCode,
                locId:locId
            },//{hisDrugCode:data.data.hisDrugCode}
            callback:(data) => {

                this.setState({
                    goodsTypeSelect2:data.data
                })
                console.log(this.state.goodsTypeSelect2)
            }
        })
    }
    goodsRender = () => {
        const {goodsList, fillBackData,goodsTypeSelect2} = this.state;
                            ...
                            <FormItem {...inlineFormItemLayout} label={item.name}>
                                ...
                                </Tooltip>
                                {
                                    getFieldDecorator(item.dataIndex,{
                                        initialValue:fillBackData[item.dataIndex],
                                        rules:[
                                            {required:item.name=='备选上架货位'?false:true,message: `请选择${item.name}！`}
                                        ]
                                    })(
                                        <Select
                                            style={{
                                            ...
                                            onChange={item.name=='补货指示货位'?this.handleProvinceChange:null}
                                            showSearch
                                            optionFilterProp="children"
                                            filterOption={(input, option) => option.props.children.toLowerCase().indexOf(input.toLowerCase()) >= 0}
                                        >
                                            {
                                                item.list && item.list.length&& item.name!='备选上架货位'?
                                                    item.list.map((goodsItem)=>(
                                                        <Option key={goodsItem.id} value={goodsItem.id}>{goodsItem.positionName}</Option>
                                                    )):
                                                    goodsTypeSelect2.map((item,index)=>(
                        ...
                        <Col span={10} style={{paddingLeft: 15}}>
                            {
                                item.name!='备选上架货位'?
                                    <div className="ant-form-item-label-left ant-col-xs-24 ant-col-sm-24 ant-col-lg-10">
                                        <label>存储单位</label>
                                    </div>:<p style={{fontSize:13}}>备选上架货位与补货指示货位不能相同</p>
                            }
                            <div className="ant-form-item-control-wrapper ant-col-xs-24 ant-col-sm-24 ant-col-lg-14">
                                <div className='ant-form-item-control'>{item.dataIndexUnit?item.dataIndexUnit:null}</div>
                            </div>
                        </Col>
                    </Row>
                </Col>
        ...
        if(option){
            Comfirm({
                ...
                onOk:()=>{
                    ...
                    dispatch({
                        type:'base/locAdjustStock',
                        payload:{
                            drugCode:adjust.drugCode,
                            deptCode:adjust.deptCode,
                            sourceLocCode:adjust.sourceLocCode,
                            targetLocCode:value,targetLocName:option.props.children
                        },
                ...
                <Form className='leftLable'>
                    ...
                    >
                        ...
                        <Panel header="库存上下限" key="2" style={customPanelStyle}>
                            <Row>
                                <Col span={16}>
                                    <FormItem {...formItemLayout} label={'是否设置库存上下限'}>
                                        {
                                    ...
                                    <FormItem {...formItemLayout} label={`本部门上限`}>
                                        {
                                            getFieldDecorator(`upperQuantity`,{
                                                ...
                                                rules:[
                                                    {required:isShow?true:false,message:'请选择本部门上限！'}
                                                ]
                                            })(
                                                <InputNumber
                                    ...
                                    <FormItem {...formItemLayout} label={`补货量`}>
                                        {
                                            getFieldDecorator(`purchaseQuantity`,{
                                                ...
                                                rules:[
                                                    {required:isShow?true:false,message:'请选择补货量！'}
                                                ]
                                            })(
                                                <InputNumber
                                                    precision={0}
                                    ...
                                    <FormItem {...formItemLayout} label={`本部门下限`}>
                                        {
                                            getFieldDecorator(`downQuantity`,{
                                                ...
                                                rules:[
                                                    {required:isShow?true:false,message:'请选择本部门下限！'}
                                                ]
                                            })(
                                                <InputNumber
                                                    precision={0}
