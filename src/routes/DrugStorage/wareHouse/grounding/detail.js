  ...
  render(){
    ...
    const notColumns = [
    {
        ...
        render: (record, row, index) => {
            ...
            if (record.storeType=='补货指示货位'&&filterId.length) {
                obj.props.rowSpan = 2;
            }
          ...
          <Row style={{display: 'flex', alignItems: 'center'}}>
              <Col span={12} style={{ marginLeft: 4 }}>
                <FetchSelect
                  ...
                  url={TableSearchUrl.searchgroundingwareHouse}
                  cb={(value, option) => {
                    if(defaultActive==1){
                        this.setState({
                            listwsjQuery:{...this.state.listwsjQuery,hisDrugCodeList:value || ''}
                        })
                    }else {
                        this.setState({
                            listysjQuery:{...this.state.listysjQuery,hisDrugCodeList:value || ''},
                        })
                    }
                  }}
                />
