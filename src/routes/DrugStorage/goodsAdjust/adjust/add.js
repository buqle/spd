...
import {Row, message, InputNumber, Col, Button, Table, Modal, Icon, Tooltip,Checkbox} from 'antd';
...
class NewAddGoodsAdjust extends PureComponent{
  ...
  submit = () => {
    let {dataSource,checkeded} = this.state;
    ...
    let goodsLocationDetailDtoList = dataSource.map(item=>{
      ...
      Conform({
          content:checkeded?"您确定要执行修改补货指示货位操作？":'您确定要执行移库操作?',
          onOk:()=>{
  ...
  changeGoalLoc = (value, i, data) => {
    ...
    if(data){
        ...
        dataSource[i].targetType = data.targetType;
        if(dataSource[i].targetType!=1){
            message.error('此商品不能修改补货指示货位');
        }
        this.setState({
    ...
    onCheckChanage = (e) =>{
        ...
        this.setState({checkeded:e.target.checked})
        if(e.target.checked){
            dataSource[index].isEditLoc =1;
        }else {
            dataSource[index].isEditLoc =0;
        }
  ...
  render(){
    const {visible, value, query, dataSource, okLoading, submitLoading,showv} = this.state;
    const columns = [
        {
        title: '序号',
        width: 60,
      ...
      },
        {
            ...
            render: (text, row, index) =>
                <div style={{textAlign:'center'}}>
                    <Checkbox
                        disabled={row.targetType!=1? true : false}
                        dataIndex={index}
