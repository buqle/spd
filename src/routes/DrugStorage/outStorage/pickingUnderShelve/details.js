...
class DetailsPickSoldOut extends PureComponent{
  constructor(props) {
    ...
    this.state = {
      ...
      pendingQuery: {
        pickingOrderNo,
        pickingStatus: 0,hisDrugCodeList:''
      },
      pickedQuery: {
        pickingOrderNo,
        pickingStatus: 1,hisDrugCodeList:''
      }
    }
  }
        ...
        <Row style={{display: 'flex', alignItems: 'center'}}>
              <Col span={12} style={{ marginLeft: 4 }}>
                <FetchSelect
                  ...
                  url={TableSearchUrl.searchpickingUnderShelve}
                  cb={(value, option) => {
                    if(activeKey==1){
                        this.setState({
                            pendingQuery:{...this.state.pendingQuery,hisDrugCodeList:value}
                        })
                    }else {
                        this.setState({
                            pickedQuery:{...this.state.pickedQuery,hisDrugCodeList:value}
                        })
                    }
                  }}
                />
