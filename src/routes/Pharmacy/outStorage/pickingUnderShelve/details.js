...
class DetailsPickSoldOut extends PureComponent{
  ...
  print = () => {
    ...
    printA = () => {
        const {pickingOrderNo} = this.props.match.params;
        window.open(`${outStorage.PICKING_PRINT_A}?pickingOrderNo=${pickingOrderNo}`, '_blank');
    }
      ...
      <div className='fadeIn' style={{ padding: '0 16px' }}>
        <Row>
          ...
          <Col span={12} style={{textAlign: 'right'}}>
            ...
            <Button icon='printer' onClick={this.print} style={{marginRight: 8}}>针式打印</Button>
            <Button icon='printer' onClick={this.printA}>A4打印</Button>
          </Col>
        </Row>
        <Row>
          <Col span={8}>
        ...
        <Row style={{display: 'flex', alignItems: 'center'}}>
          <Col span={12} style={{ marginLeft: 4 }}>
            <FetchSelect
              ...
              placeholder='药品名称'
              deptStateb={' '}
              url={TableSearchUrl.searchoutStoragepickingUnderShelve}
              cb={(value, option) => {
                let {pendingQuery,pickedQuery} = this.state;
                pendingQuery = {
                  ...pendingQuery,
